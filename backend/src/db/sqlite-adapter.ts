...
const BOOL_COLS: Record<string, string[]> = {
  ...
  run_history:             ['is_alive', 'sandbox_ok', 'is_partial'],
  mods:                    ['is_required'],
  seasons:                 ['is_active'],
...
const ALLOWED_TABLES = new Set(['players', 'moderators', 'moderator_tokens', 'entries', 'mods', 'mod_dependencies', 'player_tokens', 'seasons', 'hall_of_fame', 'daily_news', 'season_finances', 'achievements', 'player_achievements', 'heatmap_events', 'player_likes', 'journal_events', 'run_history', 'banned_identities', 'run_history', 'financial_transactions', 'prize_fund', 'prize_distribution']);

const ALLOWED_COLS: Record<string, Set<string>> = {
  players:          new Set(['id','nick','email','password_hash','email_verified_at','twitch_url','youtube_url','kick_url','tiktok_url','status','blocked','blocked_reason','blocked_at','blocked_by','blocked_note','is_supporter','supporter_until','is_test_mod','is_featured_streamer','is_moderator','player_token','created_at','deleted_at','gender','yt_channel_id','yt_sub_expires_at','yt_last_live_video_id','yt_live_confirmed_at','twitch_last_live_id','terms_accepted_at','ban_match','yt_resolve_attempts','yt_resolve_failed_at']),
  ...
  run_history:      new Set(['id','entry_id','player_id','season_id','name','character_name','profession','days','time_raw','time_str','kills','score','skills','traits','objectives','is_alive','sandbox_ok','disqualification_reason','death_cause','death_region','weapon_kills','start_region','animals_killed','fish_caught','crops_harvested','items_crafted','houses_looted','hours_without_sleep','trees_cut','books_read','structures_built','crops_planted','spiffo_visited','eggs_collected','milk_produced','stone_structures','ceramic_items','forged_weapons','km_driven','cities_visited','military_visited','meals_cooked','water_collected','materials_crafted','animal_tracks','weapons_crafted','furniture_crafted','clothes_crafted','cheese_produced','doors_opened','sleep_locations','basements_explored','stations_used','animal_species','days_no_canned','stats_synced_at','run_started_at','run_ended_at','source','is_partial','created_at']),
  banned_identities: new Set(['id','kind','value','reason','created_by','created_at']),
...
function runMigrations(db: Database): void {
  ...
  if (!entryCols.includes('disqualification_note')) {
    db.exec('ALTER TABLE entries ADD COLUMN disqualification_note TEXT DEFAULT NULL');
    console.log('[SQLite] migração: coluna disqualification_note adicionada em entries');
  }
  if (!entryCols.includes('disqualified_by')) {
    db.exec('ALTER TABLE entries ADD COLUMN disqualified_by TEXT DEFAULT NULL');
    console.log('[SQLite] migração: coluna disqualified_by adicionada em entries');
  }
  ...
  if (!entryCols.includes('active_mods')) {
    db.exec('ALTER TABLE entries ADD COLUMN active_mods TEXT DEFAULT NULL');
    console.log('[SQLite] migração: coluna active_mods adicionada em entries');
  }
  for (const [col, type] of [['death_cause', 'TEXT'], ['run_started_at', 'TEXT'], ['death_region', 'TEXT'], ['heatmap_batch', 'TEXT'], ['weapon_kills', 'TEXT'], ['start_region', 'TEXT']] as const) {
    if (!entryCols.includes(col)) {
      db.exec(`ALTER TABLE entries ADD COLUMN ${col} ${type} DEFAULT NULL`);
      console.log(`[SQLite] migração: coluna ${col} adicionada em entries`);
    }
  }
  const historyCols = (db.prepare('PRAGMA table_info(run_history)').all() as { name: string }[]).map(c => c.name);
  for (const col of ['death_region', 'weapon_kills', 'start_region']) {
    if (historyCols.length > 0 && !historyCols.includes(col)) {
      db.exec(`ALTER TABLE run_history ADD COLUMN ${col} TEXT DEFAULT NULL`);
      console.log(`[SQLite] migração: coluna ${col} adicionada em run_history`);
    }
  }
  const playerColsNow = (db.prepare('PRAGMA table_info(players)').all() as { name: string }[]).map(c => c.name);
  if (!playerColsNow.includes('yt_resolve_attempts')) {
    db.exec('ALTER TABLE players ADD COLUMN yt_resolve_attempts INTEGER NOT NULL DEFAULT 0');
    db.exec('ALTER TABLE players ADD COLUMN yt_resolve_failed_at TEXT DEFAULT NULL');
    console.log('[SQLite] migração: colunas yt_resolve_* adicionadas em players');
  }
  if (!playerColsNow.includes('ban_match')) {
    db.exec('ALTER TABLE players ADD COLUMN ban_match TEXT DEFAULT NULL');
    console.log('[SQLite] migração: coluna ban_match adicionada em players');
  }
  if (!entryCols.includes('stats_synced_at')) {
    db.exec('ALTER TABLE entries ADD COLUMN stats_synced_at TEXT DEFAULT NULL');
    console.log('[SQLite] migração: coluna stats_synced_at adicionada em entries');
  }
  if (!entryCols.includes('mod_version')) {
    db.exec('ALTER TABLE entries ADD COLUMN mod_version TEXT DEFAULT NULL');
    console.log('[SQLite] migração: coluna mod_version adicionada em entries');
  }
  ...
  if (!modsCols.includes('workshop_id')) {
    db.exec('ALTER TABLE mods ADD COLUMN workshop_id TEXT DEFAULT NULL');
    console.log('[SQLite] migração: coluna workshop_id adicionada em mods');
  }
  if (!modsCols.includes('block_reason')) {
    db.exec('ALTER TABLE mods ADD COLUMN block_reason TEXT DEFAULT NULL');
    console.log('[SQLite] migração: coluna block_reason adicionada em mods');
  }
